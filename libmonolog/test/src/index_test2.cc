...
class Index2Test : public testing::Test {
 public:
  ...
  void index_test(INDEX& index, uint64_t step = 1) {
    ...
    for (uint64_t i = 0; i < max; i += step) {
      monolog::entry_list* list = index.get(i);
  ...
  size_t indexlet_size(size_t size) {
    return size * (sizeof(monolog::entry_list*) + value_size());
  }
...
TEST_F(Index2Test, IndexletTest) {
  ...
  monolog::indexlet<uint32_wrapper> ilet;
...
TEST_F(Index2Test, Index2AddFetchTest) {
  ...
  for (uint32_t num_threads = 1; num_threads <= 4; num_threads++) {
    monolog::__index2 idx;
    index_test_mt(idx, num_threads);
  }
}

TEST_F(Index2Test, Index3AddFetchTest) {
  monolog::__index3 index;

  index_test(index, 1);
  size_t storage_size = index.storage_size();
  size_t expected_size = layer_size<monolog::__index_depth1<256>>(1)
      + 256 * indexlet_size(256);
  ASSERT_EQ(expected_size, storage_size);

  for (uint32_t num_threads = 1; num_threads <= 4; num_threads++) {
    monolog::__index3 idx;
    index_test_mt(idx, num_threads);
  }
}

TEST_F(Index2Test, Index4AddFetchTest) {
  monolog::__index4 index;

  index_test(index, 1);
  size_t storage_size = index.storage_size();
  size_t expected_size = layer_size<monolog::__index_depth1<65536>>(1)
      + indexlet_size(65536);
  ASSERT_EQ(expected_size, storage_size);

  for (uint32_t num_threads = 1; num_threads <= 4; num_threads++) {
    monolog::__index4 idx;
    index_test_mt(idx, num_threads);
  }
}

TEST_F(Index2Test, Index5AddFetchTest) {
  monolog::__index5 index;

  index_test(index, 1);
  size_t storage_size = index.storage_size();
  size_t expected_size = layer_size<monolog::__index_depth2<65536, 256>>(1)
      + layer_size<monolog::__index_depth1<256>>(1) + 256 * indexlet_size(256);
  ASSERT_EQ(expected_size, storage_size);

  for (uint32_t num_threads = 1; num_threads <= 4; num_threads++) {
    monolog::__index5 idx;
    index_test_mt(idx, num_threads);
  }
}

TEST_F(Index2Test, Index6AddFetchTest) {
  monolog::__index6 index;

  index_test(index, 1);
  size_t storage_size = index.storage_size();
  size_t expected_size = layer_size<monolog::__index_depth2<65536, 65536>>(1)
      + layer_size<monolog::__index_depth1<65536>>(1) + indexlet_size(65536);
  ASSERT_EQ(expected_size, storage_size);

  for (uint32_t num_threads = 1; num_threads <= 4; num_threads++) {
    monolog::__index6 idx;
    index_test_mt(idx, num_threads);
  }
}

TEST_F(Index2Test, Index7AddFetchTest) {
  monolog::__index7 index;

  index_test(index, 1);
  size_t storage_size = index.storage_size();
  size_t expected_size = layer_size<monolog::__index_depth3<65536, 65536, 256>>(1)
      + layer_size<monolog::__index_depth2<65536, 256>>(1)
      + layer_size<monolog::__index_depth1<256>>(1) + 256 * indexlet_size(256);
  ASSERT_EQ(expected_size, storage_size);

  for (uint32_t num_threads = 1; num_threads <= 4; num_threads++) {
    monolog::__index7 idx;
    index_test_mt(idx, num_threads);
  }
}

TEST_F(Index2Test, Index8AddFetchTest) {
  monolog::__index8 index;

  index_test(index, 1);
  size_t storage_size = index.storage_size();
  size_t expected_size = layer_size<monolog::__index_depth3<65536, 65536, 65536>>(
      1) + layer_size<monolog::__index_depth2<65536, 65536>>(1)
      + layer_size<monolog::__index_depth1<65536>>(1) + indexlet_size(65536);
  ASSERT_EQ(expected_size, storage_size);

  for (uint32_t num_threads = 1; num_threads <= 4; num_threads++) {
    monolog::__index8 idx;
    index_test_mt(idx, num_threads);
  }
}

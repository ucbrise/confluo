...
class filter_aggregates_archival_action {
 ...
 public:
  ...
  filter_aggregates_archival_action(byte_string byte_str)
      : filter_aggregates_archival_action(byte_str.size(), byte_str.data()) {
  }

  filter_aggregates_archival_action(size_t key_size, uint8_t* archival_tail_key) {
    action_ = "";
  ...
  byte_string archival_tail_key() {
    return byte_string(action_.substr(sizeof(size_t), this->key_size()));
  }

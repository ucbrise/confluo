...
class MonologLinearLoadTest : public testing::Test {
 ...
 public:
  ...
  static const uint64_t ARRAY_SIZE = MAX_BLOCKS * BUCKET_SIZE;

  typedef storage::encoded_ptr<uint8_t> encoded_ptr_t;
  typedef storage::read_only_encoded_ptr<uint8_t> read_only_ptr_t;
  typedef monolog_linear<uint8_t, MAX_BLOCKS, BUCKET_SIZE, BUF_SIZE> small_monolog_linear;
  typedef archival::monolog_linear_archiver<uint8_t, MAX_BLOCKS, BUCKET_SIZE, BUF_SIZE> small_monolog_archiver;
...
TEST_F(MonologLinearLoadTest, FullyArchivedLogLoadTest) {
  ...
  archival::monolog_linear_load_utils::load<uint8_t, MAX_BLOCKS, BUCKET_SIZE, BUF_SIZE>(path, recovered_log);
  verify(recovered_log, 0, ARRAY_SIZE);
}

//TEST_F(MonologLinearLoadTest, PartiallyArchivedLoadTest) {
//
//  std::string path = "/tmp/data_log/";
//
//  {
...
//    small_monolog_archiver archiver(path, &log);
//    write_to_log(log);
//    archiver.archive(7 * 1024);
//    verify(log, 0, 7 * 1024);
//  }
//
//  small_monolog_linear recovered_log("log", "/tmp", storage::IN_MEMORY);
//  archival::load_utils::load_monolog_linear<uint8_t, MAX_BLOCKS, BUCKET_SIZE, BUF_SIZE>(path, recovered_log);
//
//  verify(recovered_log, 0, 7 * 1024);
//
//}

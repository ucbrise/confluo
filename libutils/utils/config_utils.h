...
class configuration_map {
 public:
  ...
  configuration_map(const std::string& path)
      : path_(utils::file_utils::full_path(path)) {
    extract_all();
  }
 ...
 private:
  ...
  void extract_all() {
    std::ifstream cfile;
    cfile.open(path_.c_str());
    if (!cfile)
      LOG_WARN << "Configuration file " << path_
               << " couldn't be found; using default configurations";

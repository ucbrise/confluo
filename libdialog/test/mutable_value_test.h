...
TEST_F(MutableValueTest, FloatValueTest) {
  ...
  ASSERT_TRUE(mutable_value_t(static_cast<float>(5.2)) == (+n1));
  ASSERT_TRUE(
      approx_equals<float>(mutable_value_t(static_cast<float>(7.8)),
                           (n1 + n2)));
  ASSERT_TRUE(
      approx_equals<float>(mutable_value_t(static_cast<float>(2.6)),
                           (n1 - n2)));
  ASSERT_TRUE(
      approx_equals<float>(mutable_value_t(static_cast<float>(13.52)),
                           (n1 * n2)));
  ASSERT_TRUE(
      approx_equals<float>(mutable_value_t(static_cast<float>(2.0)),
                           (n1 / n2)));
...
TEST_F(MutableValueTest, DoubleValueTest) {
  mutable_value_t n1(static_cast<double>(5.2)), n2(static_cast<double>(2.6)),
  ...
  ASSERT_TRUE(mutable_value_t(static_cast<double>(5.2)) == (+n1));
  ASSERT_TRUE(
      approx_equals<double>(mutable_value_t(static_cast<double>(7.8)),
                            (n1 + n2)));
  ASSERT_TRUE(
      approx_equals<double>(mutable_value_t(static_cast<double>(2.6)),
                            (n1 - n2)));
  ASSERT_TRUE(
      approx_equals<double>(mutable_value_t(static_cast<double>(13.52)),
                            (n1 * n2)));
  ASSERT_TRUE(
      approx_equals<double>(mutable_value_t(static_cast<double>(2.0)),
                            (n1 / n2)));
...
TEST_F(MutableValueTest, StringValueTest) {
  mutable_value_t n1(std::string("abc")), n2(std::string("123")), n3(
      std::string("abc"));
...
TEST_F(MutableValueTest, BoolParseTest) {
  mutable_value_t val1 = mutable_value_t::parse("true", BOOL_TYPE);
  ASSERT_EQ(true, *reinterpret_cast<const bool*>(val1.ptr()));

  mutable_value_t val2 = mutable_value_t::parse("FALSE", BOOL_TYPE);
  ASSERT_EQ(false, *reinterpret_cast<const bool*>(val2.ptr()));

  ASSERT_TRUE(test::test_utils::test_fail([]() {
    mutable_value_t::parse("0", BOOL_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([]() {
    mutable_value_t::parse("true2", BOOL_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([]() {
    mutable_value_t::parse("t", BOOL_TYPE);
  }));
}

TEST_F(MutableValueTest, CharParseTest) {
  mutable_value_t val1 = mutable_value_t::parse("c", CHAR_TYPE);
  ASSERT_EQ('c', *reinterpret_cast<const char*>(val1.ptr()));

  mutable_value_t val2 = mutable_value_t::parse("1", CHAR_TYPE);
  ASSERT_EQ('1', *reinterpret_cast<const char*>(val2.ptr()));

  ASSERT_TRUE(test::test_utils::test_fail([]() {
    mutable_value_t::parse("cc", CHAR_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([]() {
    mutable_value_t::parse("11", CHAR_TYPE);
  }));
}

TEST_F(MutableValueTest, ShortParseTest) {
  mutable_value_t val1 = mutable_value_t::parse("10", SHORT_TYPE);
  ASSERT_EQ(10, *reinterpret_cast<const short*>(val1.ptr()));

  mutable_value_t val2 = mutable_value_t::parse("-100", SHORT_TYPE);
  ASSERT_EQ(-100, *reinterpret_cast<const short*>(val2.ptr()));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("10c", SHORT_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("1.0", SHORT_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("ten", SHORT_TYPE);
  }));
}

TEST_F(MutableValueTest, IntParseTest) {
  mutable_value_t val1 = mutable_value_t::parse("65536", INT_TYPE);
  ASSERT_EQ(65536, *reinterpret_cast<const int*>(val1.ptr()));

  mutable_value_t val2 = mutable_value_t::parse("-100", INT_TYPE);
  ASSERT_EQ(-100, *reinterpret_cast<const int*>(val2.ptr()));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("10c", INT_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("1.0", INT_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("ten", INT_TYPE);
  }));
}

TEST_F(MutableValueTest, LongParseTest) {
  mutable_value_t val1 = mutable_value_t::parse("2147483648", LONG_TYPE);
  ASSERT_EQ(2147483648, *reinterpret_cast<const long*>(val1.ptr()));

  mutable_value_t val2 = mutable_value_t::parse("-100", LONG_TYPE);
  ASSERT_EQ(-100, *reinterpret_cast<const long*>(val2.ptr()));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("10c", LONG_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("1.0", LONG_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("ten", LONG_TYPE);
  }));
}

TEST_F(MutableValueTest, FloatParseTest) {
  mutable_value_t val1 = mutable_value_t::parse("10.4", FLOAT_TYPE);
  ASSERT_EQ(static_cast<float>(10.4),
            *reinterpret_cast<const float*>(val1.ptr()));

  mutable_value_t val2 = mutable_value_t::parse("-100.3", FLOAT_TYPE);
  ASSERT_EQ(static_cast<float>(-100.3),
            *reinterpret_cast<const float*>(val2.ptr()));

  mutable_value_t val3 = mutable_value_t::parse("-100", FLOAT_TYPE);
  ASSERT_EQ(static_cast<float>(-100.0),
            *reinterpret_cast<const float*>(val3.ptr()));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("10.0c", FLOAT_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("ten point one", FLOAT_TYPE);
  }));
}

TEST_F(MutableValueTest, DoubleParseTest) {
  mutable_value_t val1 = mutable_value_t::parse("10.4", DOUBLE_TYPE);
  ASSERT_EQ(static_cast<double>(10.4),
            *reinterpret_cast<const double*>(val1.ptr()));

  mutable_value_t val2 = mutable_value_t::parse("-100.3", DOUBLE_TYPE);
  ASSERT_EQ(static_cast<double>(-100.3),
            *reinterpret_cast<const double*>(val2.ptr()));

  mutable_value_t val3 = mutable_value_t::parse("-100", DOUBLE_TYPE);
  ASSERT_EQ(static_cast<double>(-100.0),
            *reinterpret_cast<const double*>(val3.ptr()));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("10.0c", DOUBLE_TYPE);
  }));

  ASSERT_TRUE(test::test_utils::test_fail([] {
    mutable_value_t::parse("ten point one", DOUBLE_TYPE);
  }));
}

TEST_F(MutableValueTest, StringParseTest) {
  mutable_value_t val1 = mutable_value_t::parse("abc", STRING_TYPE(64));
  ASSERT_TRUE(strcmp("abc", reinterpret_cast<const char*>(val1.ptr())) == 0);

  mutable_value_t val2 = mutable_value_t::parse("123", STRING_TYPE(64));

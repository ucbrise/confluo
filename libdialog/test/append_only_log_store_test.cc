...
TEST_F(LogStoreTest, AppendAndGetTest1) {
  dialog::log_store<in_memory, read_stalled> ls;
  test_append_and_get(ls);
}

TEST_F(LogStoreTest, AppendAndGetTest2) {
  dialog::log_store<in_memory, write_stalled> ls;
  test_append_and_get(ls);
}

TEST_F(LogStoreTest, AppendAndGetTest3) {
  dialog::log_store<durable, read_stalled> ls("/tmp");
  test_append_and_get(ls);
}

TEST_F(LogStoreTest, AppendAndGetTest4) {
  dialog::log_store<durable, write_stalled> ls("/tmp");
  test_append_and_get(ls);
}
